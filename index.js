...
const X = () => {
  ...
  const cylinderVertices = cylinder(undefined, 1.6, undefined);
...
const run = async () => {
  ...
  let hiddenIndices = {
    Torus: new Array(9).fill(false), // Torus group (Total 9 instances)
    X: new Array(9).fill(false), // X group (Total 9 instances)
  };
  ...
  const toggleInstances = (index) => {
    if (index >= 1 && index <= 9) {
      ...
      hiddenIndices = {
        Torus: new Array(9).fill(true),
        X: new Array(9).fill(true),
      };
    }
  ...
  canvas.addEventListener("click", async (event) => {
    ...
    const cellSize = (gridSize - 2 * offset - gap * 2) / 3;
    ...
    if (row >= 0 && row < 3 && col >= 0 && col < 3) {
      ...
      console.log(`Clicked cell number: ${index}`);
